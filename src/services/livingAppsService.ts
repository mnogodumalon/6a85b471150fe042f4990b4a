...
import type { Serviceanfrage, Auftragsverwaltung, CreateServiceanfrage, CreateAuftragsverwaltung } from '@/types/app';
...
export class LivingAppsService {
  // --- SERVICEANFRAGE ---
  static async getServiceanfrage(): Promise<Serviceanfrage[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SERVICEANFRAGE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Serviceanfrage[];
    return hydrateRecords(records, 'serviceanfrage');
  }
  static async getServiceanfrageEntry(id: string): Promise<Serviceanfrage | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SERVICEANFRAGE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Serviceanfrage;
    return hydrateRecords([record], 'serviceanfrage')[0];
  }
  static async createServiceanfrageEntry(fields: CreateServiceanfrage): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SERVICEANFRAGE}/records`, { fields: cleanFieldsForApi(fields as any, 'serviceanfrage') });
    return { ...data, record_id: data.id };
  }
  static async updateServiceanfrageEntry(id: string, fields: Partial<CreateServiceanfrage>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SERVICEANFRAGE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'serviceanfrage') });
    return { ...data, record_id: data.id };
  }
  static async deleteServiceanfrageEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SERVICEANFRAGE}/records/${id}`);
  }
  ...
  static async getAuftragsverwaltung(): Promise<Auftragsverwaltung[]> {
    ...
    return hydrateRecords(records, 'auftragsverwaltung');
  }
  static async getAuftragsverwaltungEntry(id: string): Promise<Auftragsverwaltung | undefined> {
    ...
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Auftragsverwaltung;
    return hydrateRecords([record], 'auftragsverwaltung')[0];
  }
  static async createAuftragsverwaltungEntry(fields: CreateAuftragsverwaltung): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.AUFTRAGSVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'auftragsverwaltung') });
    return { ...data, record_id: data.id };
  }
  static async updateAuftragsverwaltungEntry(id: string, fields: Partial<CreateAuftragsverwaltung>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.AUFTRAGSVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'auftragsverwaltung') });
    return { ...data, record_id: data.id };
  }
