...
import { ServiceanfrageViewDialog } from '@/components/dialogs/ServiceanfrageViewDialog';
...
const SERVICEANFRAGE_FIELDS = [
  { key: 'problembeschreibung', label: 'Problembeschreibung', type: 'string/textarea' },
  { key: 'dringlichkeit', label: 'Dringlichkeit', type: 'lookup/radio', options: [{ key: 'nicht_dringend', label: 'Nicht dringend – kann warten' }, { key: 'normal', label: 'Normal – innerhalb der nächsten Tage' }, { key: 'dringend', label: 'Dringend – so bald wie möglich' }, { key: 'notfall', label: 'Notfall – sofortiger Einsatz nötig' }] },
  ...
  { key: 'ort', label: 'Stadt / Ort', type: 'string/text' },
  { key: 'einsatzort_geo', label: 'Standort auf der Karte (optional)', type: 'geo' },
  ...
  { key: 'erreichbarkeit', label: 'Beste Erreichbarkeit', type: 'string/text' },
];
const AUFTRAGSVERWALTUNG_FIELDS = [
  { key: 'anfrage', label: 'Zugehörige Anfrage', type: 'applookup/select', targetEntity: 'serviceanfrage', targetAppId: 'SERVICEANFRAGE', displayField: 'strasse' },
...
const ENTITY_TABS = [
  { key: 'serviceanfrage', pascal: 'Serviceanfrage' },
  { key: 'auftragsverwaltung', pascal: 'Auftragsverwaltung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('serviceanfrage');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'serviceanfrage': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'serviceanfrage': return (data as any).serviceanfrage as Serviceanfrage[] ?? [];
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'auftragsverwaltung':
        lists.serviceanfrageList = (data as any).serviceanfrage ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'auftragsverwaltung' && fieldKey === 'anfrage') {
      const match = (lists.serviceanfrageList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.strasse ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'serviceanfrage': return SERVICEANFRAGE_FIELDS as any[];
        case 'auftragsverwaltung': return AUFTRAGSVERWALTUNG_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'serviceanfrage': return {
        create: (fields: any) => LivingAppsService.createServiceanfrageEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateServiceanfrageEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteServiceanfrageEntry(id),
      };
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'serviceanfrage' || dialogState?.entity === 'serviceanfrage') && (
        <ServiceanfrageDialog
          open={createEntity === 'serviceanfrage' || dialogState?.entity === 'serviceanfrage'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'serviceanfrage' ? handleUpdate : (fields: any) => handleCreate('serviceanfrage', fields)}
          defaultValues={dialogState?.entity === 'serviceanfrage' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'auftragsverwaltung' || dialogState?.entity === 'auftragsverwaltung') && (
        <AuftragsverwaltungDialog
          ...
          serviceanfrageList={(data as any).serviceanfrage ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Auftragsverwaltung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Auftragsverwaltung']}
        />
      )}
      {viewState?.entity === 'serviceanfrage' && (
        <ServiceanfrageViewDialog
          open={viewState?.entity === 'serviceanfrage'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'serviceanfrage', record: r }); }}
        />
      )}
