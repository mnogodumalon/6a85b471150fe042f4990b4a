...
import { formEnhancements } from '@/config/form-enhancements/Serviceanfrage';
import { evalComputed } from '@/config/form-enhancements/types';
import { t, appLabel, fieldLabel, localeTag, CURRENCY } from '@/i18n';

export default function ServiceanfrageDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const [record, setRecord] = useState<Serviceanfrage | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getServiceanfrage();
  ...
  async function handleUpdate(fields: Serviceanfrage['fields']) {
    if (!record) return;
    await LivingAppsService.updateServiceanfrageEntry(record.record_id, fields);
    await loadData();
    setEditing(false);
  }
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteServiceanfrageEntry(record.record_id);
    setDeleteOpen(false);
    navigate('/serviceanfrage');
  }
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/serviceanfrage')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/serviceanfrage')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.strasse ?? appLabel('serviceanfrage')} />
